...
export function ConfirmReceiveModal() {
  const { t } = useLanguage();
  const ctx = useNotify();
  if (!ctx) return null;
  const { confirmRecv, confirmRecvResponse } = ctx;
  if (!confirmRecv) return null;

  const handleConfirm = (confirmed: boolean) => {
    confirmRecvResponse(confirmed);
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" role="dialog">
      <div className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-6 shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
        <h2 className="mb-4 text-lg font-semibold text-zinc-900 dark:text-zinc-100">
          {t("confirmReceive.title")}
        </h2>
        <p className="mb-4 text-sm text-zinc-600 dark:text-zinc-400">
          {t("confirmReceive.from")}: <strong>{confirmRecv.from || "?"}</strong> ({confirmRecv.fileCount} {t("confirmReceive.files")})
        </p>
        {confirmRecv.files.length > 0 && (
          <ul className="mb-4 max-h-40 overflow-y-auto text-sm text-zinc-600 dark:text-zinc-400">
            {confirmRecv.files.slice(0, 10).map((f, i) => (
              <li key={i}>{f.fileName}</li>
            ))}
            {confirmRecv.totalFiles != null && confirmRecv.totalFiles > confirmRecv.files.length && (
              <li>+{confirmRecv.totalFiles - confirmRecv.files.length} more</li>
            )}
          </ul>
        )}
